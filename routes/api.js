...
try {
var zahirr = db.get("kocak");
} catch (e) {
	console.log('') 
}

var creatorList = ['@kocak','@kocak','@kocak','@kocak', '@kocak','@kocak','@kocak'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di :) , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di :) , key nya gratis kok gan, jadi santuyy ajaa'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------kocak'+'kocak--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'kocak') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kocak') return res.json(loghandler.invalidKey)
